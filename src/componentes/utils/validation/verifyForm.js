...
export const verifyForm = (form, type = null) => {
  ...
  if (type === "modify-user") {
    ...
    if (isNaN(format.edad)) {
      error["edad"] = "Debes ingresar tu edad";
  ...
  if (type === "register" || type === "checkout") {
    ...
    if (isNaN(format.edad) && type === "register") {
      ...
      error["edad"] =
        "Debes ser mayor de edad para registrarte en nuestra página";
    }
  }
  ...
  if (type === "cambiar-password") {
    if (!validadorForm.password.test(format.password)) {
      error["password"] =
        "La contraseña debe tener al menos 4 letras y 3 números";
    }
  }
